...
const Pagination: React.FC<Props> = ({ currentIndex, totalPages, baseUrl }) => {
    ...
    for (let i = 0; i < totalPages; i++) {
        items.push(
            <Link key={i} href={`${baseUrl}${i + 1 === 1 ? '' : i + 1}`}>
    ...
    return (
        <div className={classes.pagination}>
            <Link href={`${baseUrl}${currentIndex - 1 === 0 ? '' : currentIndex}`}>
                <a className={`${classes.item} ${currentIndex === 0 ? classes.disabled : ''}`}>&lt;</a>
            </Link>
            {items}
            <Link href={`${baseUrl}${currentIndex + 2}`}>
